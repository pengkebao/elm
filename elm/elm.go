...
func (this *ELM) getAccessToken() (string, error) {
	...
	// cache.Get("access_token", &access_token)
	...
	if this.Code == "200" {
		...
		if access_token, ok := res["access_token"]; ok {
			//expire_time := ConvInterfaceToInt64(res["expire_time"])
			//cache.Set("access_token", access_token, time.Nanosecond*time.Duration(expire_time-1000))
...
func (this *ELM) createUserSign(mReq map[string]interface{}) (string, error) {
	...
	return utils.Md5(signStrings), nil
}
